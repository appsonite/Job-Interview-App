...
- (void)viewDidLoad {
	...
	[personVideos addObject:@"Motviation?"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	[self.tableView setBackgroundColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"iphone_background_index.png"]]];
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	IndexVC_iPhone *	detailView=[[IndexVC_iPhone alloc]initWithNibName:@"IndexVC_iPhone" bundle:nil ];
	[self.navigationController pushViewController:detailView animated:YES];
	
	NSString* videoName=@"";
	
	if (selectedCategory==0) {
		videoName=[eduVideos objectAtIndex:indexPath.row];
	}
	if (selectedCategory==1) {
		videoName=[expVideos objectAtIndex:indexPath.row];
	}
	if (selectedCategory==2) {
		videoName=[profVideos objectAtIndex:indexPath.row];
	}
	if (selectedCategory==3) {
		videoName=[personVideos objectAtIndex:indexPath.row];
	}
	if (selectedCategory==4) {
		videoName=[careerVideos objectAtIndex:indexPath.row];
	}
	
	if (selectedCategory==5) {
		videoName=[ambiVideos objectAtIndex:indexPath.row];
	}
	
	if (selectedCategory==6) {
		videoName=[theJob objectAtIndex:indexPath.row];
	}

	
	[detailView loadRequestWithIndex:indexPath.row :selectedCategory :videoName];
	//[detailView release];	 
}
